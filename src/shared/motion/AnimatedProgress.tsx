...
export function AnimatedProgress({
  ...
  useEffect(() => {
    ...
    const duration = token.endsWith("ms")
      ...
      : token.endsWith("s")
        ...
        : 220;
    const safeDuration =
      Number.isFinite(duration) && duration > 0 ? duration : 220;
  ...
  return (
    <div
      ref={root}
      className={`motion-progress${compact ? " motion-progress--compact" : ""}`}
    >
      <div className="motion-progress__caption" aria-hidden="true">
        <span>{label}</span>
        <strong>
          {Math.round(reducedMotion ? target : displayValue)}{" "}
          <span>из {limit}</span>
        </strong>
      </div>
      <div
        className="motion-progress__track"
        role="progressbar"
        aria-label={label}
        aria-valuemin={0}
        aria-valuemax={limit}
      ...
      >
        <span
          className="motion-progress__fill"
          style={{ transform: `scaleX(${target / limit})` }}
        />
      </div>
    </div>
  );
