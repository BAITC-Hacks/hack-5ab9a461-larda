...
For individual requests, open `03 Endpoints by resource`: all 39 endpoints are grouped into Health, Users, Reference data, Tasks, AI, Teams, Proposals and Milestones. Fill the relevant `manual_*` collection variables with real resource IDs and the current owner task revision, edit the JSON body, then use Send. These requests do not capture variables or poll automatically; their descriptions explain access and prerequisites. Run automated scenario folders separately; the whole collection also contains these manual requests and alternative lifecycle actions.
